      ...
      <div className="venue-section__header">
        ...
        <div className="venue-section__title-container">
          ...
          <span className="venue-section__distance">
            <FormattedMessage id="distance.miles" values={{ value: metersToMiles(venue?.distance) }} />
          </span>
        </div>
      </div>
      <div>
        {movies && (
          <MovieCarousel venue={{ id: venue.id, urlName: venue.urlName }} movieList={movies} meta={venueMeta} />
        )}
      </div>
      {scrollUp && (
