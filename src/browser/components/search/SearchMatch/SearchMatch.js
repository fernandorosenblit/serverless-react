  ...
  const renderMovieMatch = () => {
    ...
    return (
      <div className="match movie">
        {imageContent && (
          <img className="poster" src={imageUrl(imageContent?.[0])} alt={imageContent?.[0].image.urlAltText} />
        )}
        <div className="data">
          <h4>{displayName}</h4>
          <div className="specs">
    ...
    return (
      <div className="match venue">
        <div className="data">
          ...
          <div className="specs">
            {lat && lng && (
              <span className="additional-info">{calculateDistance([latitude, longitude], [lat, lng])} mi.</span>
            )}
          </div>
        </div>
      </div>
