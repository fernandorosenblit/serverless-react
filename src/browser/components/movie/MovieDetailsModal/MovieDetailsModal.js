...
const MovieDetailsModal = () => {
  ...
  const { textToShow, ReadMore } = useTruncateWithEllipsis(currentMovie?.textContent?.[0]?.content ?? "");

  useEffect(() => {
    Modal.setAppElement("body");
  }, []);

  const handleCloseModal = useCallback(() => switchModalAction({ isOpen: false, movieId: null }), [switchModalAction]);
        ...
        >
          <section className="movie-details-main-section movie-details-content-wrapper__separator">
            ...
            <div className="movie-details-main-section__right-wrapper">
              <span className="movie-details-main-section__title-wrapper">
                <BookmarkIcon />
                <span className="movie-details-main-section__film-title">{currentMovie?.displayName}</span>
              </span>
              ...
              <div>
                ...
                <span className="movie-details-main-section__duration">{runtimeToHs(currentMovie?.runtime)}</span>
              </div>
          ...
          <section className="movie-details-info-section movie-details-content-wrapper__separator">
            <div className="movie-details-info-section__left-wrapper">
              <h3 className="movie-details-info-section__header">{intl.formatMessage(labels.releaseDate)}</h3>
              ...
              <h3 className="movie-details-info-section__header">{intl.formatMessage(labels.genre)}</h3>
              <span className="movie-details-info-section__text movie-details-info-section__text--white">
                {currentMovie?.genres && currentMovie?.genres[0]?.displayName}
              </span>
            </div>
            <div className="movie-details-info-section__right-wrapper">
              <h3 className="movie-details-info-section__header">{intl.formatMessage(labels.sypnosys)}</h3>
              <div className="movie-details-info-section__text">{textToShow}</div>
              <ReadMore />
              <h3 className="movie-details-info-section__header">{intl.formatMessage(labels.cast)}</h3>
              <span className="movie-details-info-section__text">-</span>
              <h3 className="movie-details-info-section__header">{intl.formatMessage(labels.director)}</h3>
              <span className="movie-details-info-section__text movie-details-info-section__text--white">-</span>
            </div>
          </section>
