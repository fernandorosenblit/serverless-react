...
import { cancelOrder, goPreviousStep, resetSeatSelection, resetShowtime } from "state/actions/showtimeActions";

import WalletsContext from "contexts/walletsContext";
import GoBackArrow from "assets/icons/back-arrow.svg";
...
const ShowtimeContent = ({ currentStep, children }) => {
  ...
  const { Modal: SelectedSeatUnavailableModal, setIsOpen: setIsOpenSelectedSeatUnavailable } = usePrimaryModal();
  const {
    subHeader,
    isTimerRunning,
      ...
      <div className="showtime-content d-flex">
        <div className="showtime-content-container">
          <div className="showtime-content-container__header-wrapper">
            ...
            <h1 className="showtime-content-container__header">{intl.formatMessage({ id: stepInfo.header })}</h1>
            {isTimerRunning && <Timer />}
          </div>
          {subHeader && <span className="showtime-content-container__sub-header body2">{subHeader}</span>}
          <div
