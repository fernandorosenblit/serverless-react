...
import CreditCardIcon from "assets/icons/checkout/credit-card.svg";
import GoogleIcon from "assets/images/checkout/google-pay.svg";
import AppleIcon from "assets/images/checkout/apple-pay.svg";
...
const PaymentMethodSelector = ({ selectedMethod, onChange, availablePaymentMethods }) => {
  ...
  return (
    <div className="d-flex">
      {isCardAvailable && (
        <CheckoutPaymentMethod selected={isCardSelected} onClick={() => onChange(PAYMENT_METHOD.creditCard)}>
      ...
      {isGoogleAvailable && (
        <CheckoutPaymentMethod selected={isGoogleSelected} onClick={() => onChange(PAYMENT_METHOD.googlePay)}>
          <GoogleIcon />
        </CheckoutPaymentMethod>
      )}
      {isAppleAvailable && (
        <CheckoutPaymentMethod selected={isApplePaySelected} onClick={() => onChange(PAYMENT_METHOD.applePay)}>
          <AppleIcon />
        </CheckoutPaymentMethod>
      )}
    </div>
